...
class RNTSketchCanvasViewManager :
  ...
  override fun receiveCommand(root: RNTSketchCanvasView, commandId: String?, args: ReadableArray?) {
  ...
  override fun save(
    ...
    view?.getSketchCanvas()?.save(imageType, folder ?: "", filename, transparent, includeImage, includeText, cropToImageSize)
  }
  ...
  override fun addPath(
    ...
    if (path != null) {
      for (i in 0 until path.size()) {
        val coor =
          path.getString(i).split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
        if (pointPath != null) {
  ...
  override fun transferToBase64(
    ...
    view?.getSketchCanvas()?.getBase64(imageType, transparent, includeImage, includeText, cropToImageSize)
  }
}
